...
import com.ducatus.databinding.FragmentLoanLendDialogBinding
...
class LoanLendDialogFragment : DialogFragment() {
    private lateinit var activity: Activity
    private lateinit var auth: FirebaseAuth
    private lateinit var binding: FragmentLoanLendDialogBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentLoanLendDialogBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        amountViewModel.amount.observe(viewLifecycleOwner) { amount ->
            amount.getContentIfNotHandled()?.let { content ->
                binding.tfLoanLendAmount.editText?.setText(content)
            }
        }

        binding.tfLoanLendAmount.editText?.setOnClickListener {
            val fragmentManager = childFragmentManager
            val newFragment = AmountDialogFragment()
            newFragment.show(fragmentManager, "dialog")
        }

        binding.btnLoanLendCancel.setOnClickListener {
            dismiss()
        }

        binding.btnLoanLendSave.setOnClickListener {
            validateData()
        }
    }
    ...
    private fun loadData() {
        ...
        if (selectedLoan.paidAt == null) {
            isPaid = false
            binding.tfLoanLendDate.hint = getString(R.string.date)

            // change button text from lend to give if loan is of type borrow
            if (selectedLoan.type == "B") {
    ...
    private fun setDateTimePicker() {
        ...
        timePicker.addOnPositiveButtonClickListener {
            ...
            binding.tfLoanLendTime.editText?.setText(time)
        ...
        binding.tfLoanLendDate.editText?.setOnClickListener {
            if (!datePicker.isAdded) {
                datePicker.show(childFragmentManager, "tag")
            }
        }

        binding.tfLoanLendDate.setEndIconOnClickListener {
            if (!datePicker.isAdded) {
                datePicker.show(childFragmentManager, "tag")
            }
        }

        binding.tfLoanLendTime.editText?.setOnClickListener {
            if (!timePicker.isAdded) {
                timePicker.show(childFragmentManager, "tag")
            }
        }

        binding.tfLoanLendTime.setEndIconOnClickListener {
            if (!timePicker.isAdded) {
                timePicker.show(childFragmentManager, "tag")
            }
        }
    ...
    private fun inputObserver() {
        binding.tfLoanLendAmount.editText?.doOnTextChanged { text, _, _, _ ->
            if (text == null || text.isEmpty()) {
                binding.tfLoanLendAmount.error = getString(R.string.amount_empty)
            }
            else {
                binding.tfLoanLendAmount.error = null
            }
        }

        binding.tfLoanLendAmount.editText?.doAfterTextChanged { text ->
            if (text.toString().startsWith("0")) {
                text?.clear()
            }
        }

        binding.tfLoanLendDate.editText?.doOnTextChanged { text, _, _, _ ->
            if (text != null) binding.tfLoanLendDate.error = null
        }

        binding.tfLoanLendTime.editText?.doOnTextChanged { text, _, _, _ ->
            if (text != null) binding.tfLoanLendTime.error = null
        }
    }
    ...
    private fun validateData() {
        ...
        val amount = binding.tfLoanLendAmount.editText?.text.toString().trim { it <= ' ' }
        val date = binding.tfLoanLendDate.editText?.text.toString().trim { it <= ' ' }
        val time = binding.tfLoanLendTime.editText?.text.toString().trim { it <= ' ' }
        var notes: String? = binding.tfLoanLendNotes.editText?.text.toString().trim { it <= ' ' }
        var errors = 0

        if (TextUtils.isEmpty(date)) {
            binding.tfLoanLendDate.error = getString(R.string.date_empty)
            errors++
        }

        if (TextUtils.isEmpty(time)) {
            binding.tfLoanLendTime.error = getString(R.string.time_empty)
            errors++
        }
        ...
        if (TextUtils.isEmpty(amount)) {
            ...
            if (amount.startsWith("0")) {
                binding.tfLoanLendAmount.error = getString(R.string.amount_starts_0)
                errors++
            }
        }
    ...
    private fun showProgressDialog() {
        binding.pbLoanLend.visibility = View.VISIBLE
    ...
    private fun hideProgressDialog() {
        binding.pbLoanLend.visibility = View.INVISIBLE
