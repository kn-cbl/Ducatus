...
class UpdateMobileNumberDialogFragment : DialogFragment() {
    private lateinit var activity: Activity
    private lateinit var auth: FirebaseAuth
    private lateinit var binding: FragmentUpdateMobileNumberDialogBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentUpdateMobileNumberDialogBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvUpdateMobileResendVerify.setOnClickListener {
        ...
        databaseReference.get()
            .addOnSuccessListener {
                ...
                if (!numberKey) {
                    ...
                    hideProgressDialog()
                    binding.tfUpdateMobileNumber.error = getString(R.string.mobile_number_exists)
                }
            }
        ...
        when {
            ...
            verificationCode.length < 6 -> binding.tfVerifyMobileNumber.error = getString(R.string.verification_code_error)
                ...
                if (firebaseUser != null) {
                    updateDB(firebaseUser!!, mobileNumber, verificationCode)
                }
                else {
                    hideProgressDialog()
                    sessionExpired()
    ...
    private fun updateDB(firebaseUser: FirebaseUser, mobileNumber: String, verificationCode: String) {
        showProgressDialog()
        databaseReference = database.getReference("users").child(firebaseUser.uid).child("mobileNumber")
        databaseReference.setValue(mobileNumber)
            .addOnSuccessListener {
                ...
                updateMobileNumber(firebaseUser, phoneAuthCredential)
            }
            .addOnFailureListener {
                hideProgressDialog()
                Toast
    ...
    private fun updateMobileNumber(firebaseUser: FirebaseUser, phoneAuthCredential: PhoneAuthCredential) {
        showProgressDialog()
        ...
        callbacks = object : PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
            ...
            override fun onVerificationFailed(e: FirebaseException) {
                if (e is FirebaseAuthInvalidCredentialsException) {
                    ...
                    hideProgressDialog()
                    Toast
                        .makeText(activity, e.localizedMessage, Toast.LENGTH_LONG)
                        .show()
                }
    ...
    private fun setTimer() {
        timer = object : CountDownTimer(60000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                binding.tvUpdateMobileResendVerify.text = message
            }
    ...
    private fun enableResendButton() {
        binding.tvUpdateMobileResendVerify.setTextColor(ContextCompat.getColor(activity,R.color.green_primary))
        binding.tvUpdateMobileResendVerify.text = getString(R.string.resend_verification_code)
        binding.tvUpdateMobileResendVerify.isEnabled = true
    }

    private fun disableResendButton() {
        binding.tvUpdateMobileResendVerify.setTextColor(ContextCompat.getColor(activity,R.color.darker_gray))
        binding.tvUpdateMobileResendVerify.isEnabled = false
    }
